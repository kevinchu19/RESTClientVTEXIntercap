...
{
    public class PricesClient<TResource> : ClientBase<TResource>
    {
        public PricesClient(HttpClient httpClient, IConfigurationRoot configuration, string path, ILogger logger) :
        ...
        public async Task<bool> PutPricesAsync(TResource data, int Id, CancellationToken cancellationToken)
        {
            var contentString = JsonSerializer.Serialize(data);
            string path = $"{_path.Replace("$SkuId", Id.ToString())}";

...
{
    public class ProductsClient<TResource> : ClientBase<TResource>
    {
        ...
        public async override Task<VTEXNewIDResponse> PostWithNewIDAsync(TResource data, CancellationToken cancellationToken)
        {
            ...
            {
                ProductDTO responseContent = await JsonSerializer.DeserializeAsync<ProductDTO>(await response.Content.ReadAsStreamAsync());

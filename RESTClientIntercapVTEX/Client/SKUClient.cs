...
{
    public class SKUClient<TResource> : ClientBase<TResource>
    {
        public SKUClient(HttpClient httpClient, IConfigurationRoot configuration, string path, ILogger logger) :
        ...
        public async override Task<VTEXNewIDResponse> PostWithNewIDAsync(TResource data, CancellationToken cancellationToken)
        {
            ...
            {
                SkuDTOResponse responseContent = await JsonSerializer.DeserializeAsync<SkuDTOResponse>(await response.Content.ReadAsStreamAsync());
